...
use super::{Enclu, ErrorCode, Secinfo};
...
pub fn eaccept(page: u64, secinfo: &Secinfo) -> Result<(), ErrorCode> {
    let err: u32;
    unsafe{ asm!("enclu",
                 inout("rax") Enclu::EAccept as u32 => err,
                 in("rbx") secinfo,
                 in("rcx") page); }
    match err {
        0 => Ok(()),
        _ => Err(ErrorCode::try_from(err).unwrap()), // unwrap ok, `enclu` will always return a valid error code
    }
}
