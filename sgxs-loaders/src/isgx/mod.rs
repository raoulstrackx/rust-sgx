...
impl EnclaveLoad for InnerDevice {
    ...
    fn create_controller(mapping: &Mapping<Self>) -> Option<Self::EnclaveController> {
        EnclaveController::new(mapping.device.path.clone(), mapping.device.driver).ok()
    }
}
...
impl EnclaveController {
    pub fn new(path: Arc<PathBuf>, family: DriverFamily) -> Result<Self, Error> {
        Ok(EnclaveController{
            path,
            family,
        })
    }
